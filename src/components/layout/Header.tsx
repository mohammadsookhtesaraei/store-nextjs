...
const Header = () => {
  ...
  const { getAllProductQty } = useProduct();
      ...
      <nav className="max-w-7xl px-4 sm:px-6 lg:px-8 mx-auto ">
        <div className="flex items-center border-b border-b-gray-200 h-16 mb-8">
          ...
          <div
            className={
              isOpen
                ? "lg:hidden z-10 rounded-bl-md pb-2 bg-gray-200 w-60 transition-all duration-200 absolute top-0 right-0 border border-gray-200"
                : "absolute top-0 -right-full"
            }
          >
            <div className="text-left p-2">
              <button onClick={() => setIsOpen((prev) => !prev)}>
                <span className="text-lg text-gray-700 hover:text-gray-800">
                  x
                </span>
              </button>
            </div>
          ...
          <div className=" flex items-center gap-x-4 lg:gap-x-6 ms-auto">
            ...
            <div>
              <Link className="flex items-center p-2 group" href={"/cart"}>
                ...
                <span className="text-sm ms-2 font-medium text-gray-700 group-hover:text-gray-800 block size-5 leading-5 text-center">
                  {getAllProductQty}
                </span>
              </Link>
            </div>
